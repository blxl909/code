...
class CLSNet(nn.Module):
    def __init__(self,
        ...
        self.prototype = nn.Parameter(torch.randn(self.num_classes,self.num_prototype_per_class,self.decoder_out_channels),requires_grad=True)
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            center = normalize_class_local_center.permute(1, 0, 2) #(k, B', c)

            proto_center_dist = torch.matmul(center, self.prototype.permute(0, 2, 1)) #(k, B', m)

            nearest_indices = torch.argmax(proto_center_dist, dim=-1) #(k, B')
        ...
        x = feats.permute(0, 2, 3, 1).reshape(-1, c) #(B*H*W, C)
        ...
        pixel_proto_dist = pixel_proto_dist * 15

        pixel_proto_dist = pixel_proto_dist.view(batch_size*h_head*w_head, self.num_classes, -1).permute(1, 0, 2) #(k, B*H*W, m)
        proto_class_dist = torch.max(pixel_proto_dist, dim=-1)[0] #(k, B*H*W)
        proto_class_dist = proto_class_dist.view(self.num_classes, batch_size, h_head, w_head).permute(1, 0, 2, 3) #(B, K, H, W)

        pred_dsn = F.interpolate(pred_dsn, size=(h_org, w_org), mode="bilinear", align_corners=False)

        pred = F.interpolate(proto_class_dist, size=(h_org, w_org), mode="bilinear", align_corners=False)

        if self.training:
            return pred,coarse_pred,pred_dsn,self.prototype
        else:
            return pred,self.prototype
