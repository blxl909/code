...
class local_center_loss(nn.Module):
    ...
    def forward(self,predition, gt_mask):
        if self.training:
              pred,adaptive_class_center,self.aux, p2c_sim_map,distance_l2,prototypes = predition
        else:
              pred,adaptive_class_center, p2c_sim_map ,distance_l2,prototypes = predition
        batch_size, h, w = gt_mask.size()
...
#==============================================
        ...
        if self.training:
              ...
              return  p2c_loss + ce_loss  +  0.4 * aux_loss - 0.08 * subspace_sep + orth_cost + 0.5*distance_p2c_loss
        else:
              ...
              return  p2c_loss + ce_loss - 0.08 * subspace_sep + orth_cost +0.5* distance_p2c_loss
